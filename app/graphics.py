...
from .helpers import misc_dash_slices, TBL, update_slice_ids
...
def gen_filter(subject, comparator, operator="=="):
    return {"clause": "WHERE", "comparator": comparator, "expressionType": "SIMPLE", "operator": operator, "subject": subject, "fromFormData": True,}
def load_data(tbl_name, database):
    pdf = pd.read_json(get_example_data("birth_names.json.gz"))
    pdf.ds = pd.to_datetime(pdf.ds, unit="ms")
    pdf.to_sql(tbl_name, database.get_sqla_engine(), if_exists="replace", chunksize=500,
        		dtype={"ds": DateTime, "gender": String(16), "state": String(10), "name": String(255),},index=False,)
...
def load_birth_names(only_metadata=False, force=False):
    ...
    if not any(col.column_name == "num_california" for col in obj.columns):
        ...
        obj.columns.append(
            TableColumn(column_name="num_california", expression=f"CASE WHEN {col_state} = 'CA' THEN {col_num} ELSE 0 END",))
    ...
    obj.fetch_metadata()
    tbl = obj
    defaults = {"compare_lag": "10", "compare_suffix": "o10Y", "limit": "25", "granularity_sqla": "ds", "groupby": [], "metric": "sum__num",
        			"metrics": [{"expressionType": "SIMPLE", "column": {"column_name": "num", "type": "BIGINT"}, "aggregate": "SUM",
                	"label": "Births", "optionName": "metric_11",}], "row_limit": config["ROW_LIMIT"], "since": "100 years ago",
                	"until": "now", "viz_type": "table", "markup_type": "markdown",}
    admin = security_manager.find_user("admin")
    print("Creating some slices")


    slices = [Slice(slice_name="Participants", viz_type="big_number", datasource_type="table",
					datasource_id=tbl.id, params=get_slice_json(defaults, viz_type="big_number",
					granularity_sqla="ds", compare_lag="5", compare_suffix="over 5Y",),),
        		Slice(slice_name="Genders", viz_type="pie", datasource_type="table",
					datasource_id=tbl.id, params=get_slice_json(defaults, viz_type="pie",
				 	groupby=["gender"]),),
				Slice(slice_name="Trends", viz_type="line", datasource_type="table",
						datasource_id=tbl.id, params=get_slice_json(defaults, viz_type="line",
						groupby=["name"], granularity_sqla="ds", rich_tooltip=True, show_legend=True,),),

				Slice(slice_name="Genders by State", viz_type="dist_bar", datasource_type="table",
						datasource_id=tbl.id, params=get_slice_json(defaults, adhoc_filters=[{
                        "clause": "WHERE",
