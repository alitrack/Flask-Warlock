...
fcfg = config.instruct(pxcfg).override(cfg)#							||
#=======================================================================||
@bp.route('/')#															||
@bp.route('/<int:page>')#												||Pagination Pages
def index(page=1):#														||
	posts = Post.query.order_by(desc(Post.post_date)).paginate(page,#	||
									fcfg['index']['pagesize'], False)#	||
	categories = Category.query.all()#									||
	blogrolls = Blogroll.query.all()#									||
	return render_template('index.html', title='Home', posts=posts,#	||
							categories=categories, blogrolls=blogrolls,#||
													user=current_user)#	||
...
def get_post_by_category(category_id):#									||
	...
	posts = Post.query.filter_by(category_id=category_id).order_by(#	||
									desc(Post.post_date)).paginate(#	||
										page, POSTS_PER_PAGE, False)#	||
	categories = Category.query.all()#									||
	blogrolls = Blogroll.query.all()#									||
	return render_template('category_blog.html', title='Home',#			||
							posts=posts, categories=categories,#		||
							blogrolls=blogrolls, user=current_user)#	||
@cache.cached(timeout=50)#												||
@bp.route('/posts/<int:post_id>', methods=['GET'])#						||
@bp.route('/categories/<int:category_id>/posts/<int:post_id>',#			||
													methods=['GET'])#	||
